...
public class RemoterView2 extends View {
    ...
    public RemoterView2(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:// 移动
                //非线性取决于括号到括号外的映射
                float moveX = Smooth(event.getX() - downPointX) + mCenterPoint.x;
                float moveY = Smooth(event.getY() - downPointy) + mCenterPoint.y;
            ...
            case MotionEvent.ACTION_CANCEL:// 移出区域
                ...
                float upX = event.getX() - subpointX;
                float upY = event.getY() - subpointY;
                baseDistance = mAreaRadius+2;
                Log.e("baseDistance",baseDistance+"");
                mRockerPosition = getRockerPositionPoint(mCenterPoint, new Point((int) upX, (int) upY), mAreaRadius + mRockerRadius, mRockerRadius);
                ...
                if (mOnAngleChangeListener != null) {
                    mOnAngleChangeListener.angle(0);
                }
                if (mOnDistanceLevelListener != null) {
                    mOnDistanceLevelListener.onDistanceLevel(0);
                }
                callBackFinish();
                break;
        }
