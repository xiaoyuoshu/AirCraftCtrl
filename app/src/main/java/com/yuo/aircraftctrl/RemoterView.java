...
public class RemoterView extends View {
    ...
    private int mDistanceLevel = 100;

    //保存按下时的点
    private float downPointX;
    private float downPointy;
    private float subpointX;
    private float subpointY;
    private float lastThro = -1;
    private float cunThro;

    private boolean beforeFirstToouch = true;
    ...
    private void initAttribute(Context context, AttributeSet attrs) {
        ...
        //距离级别
        mDistanceLevel = typedArray.getInt(R.styleable.RockerView_rockerSpeedLevel, 256);
        //回调模式
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(lastThro < 0 ){
            lastThro = measuredHeight - mRockerRadius;
            cunThro = lastThro;
        }
        // 可移动区域的半径
        ...
        if (0 == mRockerPosition.x || 0 == mRockerPosition.y) {
            mRockerPosition.set(mCenterPoint.x, measuredHeight - mRockerRadius);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:// 移动
                //非线性取决于括号到括号外的映射
                float moveX = SmmothY(event.getX() - downPointX) + mCenterPoint.x;
                float moveY = Smooth(event.getY() - downPointy) + cunThro;
                ...
                lastThro = mRockerPosition.y;
                break;
            case MotionEvent.ACTION_UP:// 抬起
            case MotionEvent.ACTION_CANCEL:// 移出区域
                // 回调 结束9
                // 两点在X轴的距离
                float lenX = (float) (mCenterPoint.x - mCenterPoint.x);
                // 两点在Y轴距离
                float lenY = (float) (lastThro - mCenterPoint.y);
                // 两点距离
                float lenXY = (float) Math.sqrt((double) (lenX * lenX + lenY * lenY));
                float xylenth = Math.max(Math.abs(lenX), Math.abs(lenY));
                // 计算弧度
                double radian = Math.acos(lenX / lenXY) * (lastThro < mCenterPoint.y ? -1 : 1);
                // 计算角度
                double angle = radian2Angle(radian);
                moveRocker(mCenterPoint.x, lastThro);
    ...
    private Point getRockerPositionPoint(Point centerPoint, Point touchPoint, float regionRadius, float rockerRadius) {
        ...
        if (xylenth + rockerRadius <= regionRadius) { // 触摸位置在可活动范围内
            // 回调 返回参数
            callBack(angle, (int) lenXY);
            return touchPoint;
        } else { // 触摸位置在可活动范围以外
            // 计算要显示的位置
            //Log.e("XY", "X:"+lenX+" Y:"+lenY+" XY"+lenXY+" xylen"+xylenth);
            float newregionRadius = (float) ((regionRadius - rockerRadius)/ Math.max(Math.abs(Math.cos(radian)), Math.abs(Math.sin(radian))));
            int showPointX = (int) (centerPoint.x + newregionRadius * Math.cos(radian));
            int showPointY = (int) (centerPoint.y + newregionRadius * Math.sin(radian));
    ...
    private void callBack(double angle, float distance) {
        ...
        if (null != mOnAngleChangeListener) {
            mOnAngleChangeListener.angle(360-angle);
        }
        if (null != mOnShakeListener) {
